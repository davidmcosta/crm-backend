...
export async function getStats() {
  ...
  for (const o of allOrders) {
    ...
    if (!customerMap[id]) customerMap[id] = { name: o.customer.name, count: 0, revenue: 0 }
    customerMap[id].count++
    if (o.status === 'PAID') customerMap[id].revenue += toNum(o.valorTotal)
  }
  ...
  for (const o of allOrders) {
    ...
    for (const p of prods) {
      ...
      if (o.status === 'PAID') productMap[name].revenue += val
    }
  }
